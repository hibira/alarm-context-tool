...
アラームコンテキストツール (ACT) は、トラブルシューティングと分析に役立つ追加のコンテキストを提供することで、AWS CloudWatch Alarms を強化します。このソリューションは、Lambda、CloudWatch、X-Ray、Amazon Bedrock などの AWS サービスを活用することで、メトリクス、ログ、トレースを集約して分析し、有意義な洞察を生成します。Amazon BedrockのジェネレーティブAI機能を使用して、調査結果をまとめ、潜在的な根本原因を特定し、関連ドキュメントへのリンクを提供することで、オペレーターが問題をより効率的に解決できるようになっています。この実装は、既存のオブザーバビリティパイプラインへの展開と統合が容易になるように設計されているため、応答時間が大幅に短縮され、根本原因分析が改善されます。
...
2. IDE を使用してコード内の問題を検出する予定がある場合は、依存関係をインストールしてください。:
    ```sh
    pip install -r ./dependencies_layer/requirements.txt
    pip install aws_lambda_powertools 
    ```

3. 一部の地域では、template.yaml のコロンの後にある Lambda Insights のレイヤーバージョンを変更する必要がある場合があります。https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Lambda-Insights-extension-versionsx86-64.html を参照してください。
    ```yaml
    - !Sub arn:aws:lambda:${AWS::Region}:580247275435:layer:LambdaInsightsExtension:49
    ```

4. テンプレート.yaml ファイルを受信者のメールアドレスと送信者のアドレスで編集します。
...
5. [環境変数](#environment-variables) を更新してください

6. CloudWatch アラームから通知を受け取る SNS トピックを更新:
	- Protocol: AWS Lambda
	- Endpoint: ARN of your Lambda function

## デプロイ
1. ガイド付きデプロイメントを使用して開始する:
    ```sh
    sam build
    sam deploy --guided
    ```

2. その後、次のコマンドを使用してビルド、デプロイ、テストできます。:
    テストイベントは共有する必要があります。「テスト」を参照
    ```sh
...
## 使い方
デプロイされると、Lambda 関数は CloudWatch アラームに登録されている SNS トピックによってトリガーされます。この関数は、関連するメトリクス、ログ、トレースなどのコンテキストを追加してアラームメッセージを拡張します。Amazon Bedrock を使用して収集されたデータを分析し、実用的な洞察を生成します。
...
2. **Use the test cases generated in the logs**:
...
詳細については、[コントリビューション](CONTRIBUTING.md#security-issue-notifications) を参照してください。
